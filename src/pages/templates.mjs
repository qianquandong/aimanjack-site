...
<p class="callout" style="margin-top:24px">${s.part(parent.title, href(lang, `/workflows/${parent.slug}/`), loc({ title: '' }, lang) && (lang === 'zh' ? `${r.dept[t.category]}团队的 AI 应用` : `AI for ${r.dept[t.category]}`), href(lang, `/use-cases/${t.category}/`), L(lang, '/ai-training/'))}</p></div></section>
