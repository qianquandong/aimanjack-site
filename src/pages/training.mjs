...
const schema = (lang, home = false) => {
  ...
  if (home) {
    const prefix = `${SITE}${lang === 'zh' ? '/zh' : ''}`;
    const from = `${prefix}/ai-training/`;
    const to = `${prefix}/`;
    g = JSON.parse(JSON.stringify(g).split(from).join(to));
  }
...
export const pages = [{ path: '/ai-training/', priority: 0.8, changefreq: 'weekly',
  en: { ...trainingPage('en'), canonicalPath: '/', noindex: true },
  zh: { ...trainingPage('zh'), canonicalPath: '/', noindex: true } }];
