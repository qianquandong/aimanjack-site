...
-- Atomic fixed-window request counters for the coupon endpoint. Keys are SHA-256
-- hashes generated by the Pages Function; raw IPs and email addresses are not stored.
...
create index if not exists coupon_rate_limits_updated_at on booking.coupon_rate_limits (updated_at);
...
begin
  ...
  on conflict (key_hash) do update set
    requests = case when coupon_rate_limits.window_start <= cutoff then 1 else coupon_rate_limits.requests + 1 end,
    window_start = case when coupon_rate_limits.window_start <= cutoff then clock_timestamp() else coupon_rate_limits.window_start end,
